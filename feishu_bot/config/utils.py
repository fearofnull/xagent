...
from feishu_bot.constant import (
...
def is_running_in_container() -> bool:
    ...
    Prefer env XAGENT_RUNNING_IN_CONTAINER (1/true/yes) at call time so
    supervisord child gets correct value; else check /.dockerenv and cgroup.
    """
    if os.environ.get("XAGENT_RUNNING_IN_CONTAINER", "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
...
def _get_darwin_default_browser() -> Tuple[Optional[str], Optional[str]]:
    ...
    except (OSError, plistlib.InvalidFileException, ImportError):
        return result
